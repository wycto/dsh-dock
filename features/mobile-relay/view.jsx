...
/**
 * 局域网电脑直连（0.0.0.0）卡片：在独立端口启动一个绑定 0.0.0.0 的 dsh web 子实例，
 * 与主实例共用同一份会话与工作区，局域网电脑访问到的功能与 127.0.0.1 完全一致。
 */
function LanDirectCard() {
  const [lan, setLan] = useState(null)
  const [lanPort, setLanPort] = useState(3082)
  ...
  const lanLink = useMemo(() => {
    if (!lan || !lan.active || !lanAddress || !lan.port) return ''
    return 'http://' + lanAddress + ':' + lan.port
  }, [lan, lanAddress])
  useEffect(() => {
    let active = true
    if (!lanLink) { setLanQr(''); return () => { active = false } }
    QRCode.toDataURL(lanLink, {
      errorCorrectionLevel: 'M', margin: 2, width: 220,
      color: { dark: '#111827', light: '#ffffff' },
    }).then((value) => { if (active) setLanQr(value) }).catch(() => {})
    return () => { active = false }
  }, [lanLink])
  ...
  async function startLan() {
    ...
    if (!Number.isInteger(numericPort) || numericPort < 1024 || numericPort > 65535) { setLanMessage('直连端口需为 1024 到 65535 之间的整数。'); return }
    setLanBusy('start')
    try {
    ...
    } catch (e) { setLanMessage(e && e.message ? e.message : String(e)) } finally { setLanBusy('') }
  }
  ...
  return <section className="dmr dmr-lan">
    <div className="dmr-lan-head">
      <div className="dmr-lan-title"><span className="dmr-eyebrow"><i/> 局域网电脑直连（0.0.0.0）</span><h4>让局域网电脑访问完整 DSH</h4><p>在独立端口启动一个绑定 0.0.0.0 的 dsh web 实例，与主实例共用同一份会话与工作区；电脑端功能、交互与 127.0.0.1 完全一致（含工作区文件夹选择、会话记录）。</p></div>
      <span className={'dmr-lan-badge' + (running ? ' on' : '')} role="status">{running ? '运行中 · :' + lan.port : '未开启'}</span>
    </div>
    {running ? <div className="dmr-lan-active">
      <div className="dmr-network-grid">
        <label className="dmr-field"><span>局域网 IP</span><select value={lanAddress} onChange={(e) => setLanAddress(e.target.value)}>{lan.addresses && lan.addresses.length ? lan.addresses.map((item) => <option key={item.interface + item.address} value={item.address}>{item.address} · {item.interface}</option>) : <option value="">未检测到地址</option>}</select></label>
        <div className="dmr-field"><span>实例信息</span><input readOnly value={'0.0.0.0:' + lan.port + ' · PID ' + (lan.pid || '—')} aria-label="直连实例信息"/></div>
      </div>
      <div className="dmr-share"><div className="dmr-share-layout"><div className="dmr-qr-card">{lanQr ? <img src={lanQr} alt="局域网直连地址二维码" width="220" height="220"/> : <div className="dmr-qr-loading">正在生成二维码…</div>}<strong>扫码或输入地址访问</strong><small>电脑、手机均可直接访问</small></div><div className="dmr-share-detail"><div><span className="dmr-eyebrow"><i/> 完整 DSH 实例已运行</span><h4>{lanLink}</h4></div><div className="dmr-link" title={lanLink}>{lanLink}</div><div className="dmr-share-actions"><button type="button" className="dmr-primary" onClick={copyLanLink} disabled={!lanLink}><RelayIcon name="copy"/>复制地址</button><button type="button" className="dmr-secondary dmr-danger" onClick={stopLan} disabled={lanBusy === 'stop'}><RelayIcon name="close"/>{lanBusy === 'stop' ? '正在关闭…' : '关闭直连'}</button></div><small>此实例与主实例共用同一份会话与工作区；<strong>没有登录保护</strong>，等同官方 CLI 拒绝的 `--host 0.0.0.0`，请在可信局域网内使用，用毕及时关闭。</small></div></div></div>
    </div> : <div className="dmr-lan-idle">
      <div className="dmr-network-grid">
        <label className="dmr-field"><span>直连端口</span><input value={lanPort} onChange={(e) => setLanPort(e.target.value)} inputMode="numeric" type="number" min="1024" max="65535" aria-describedby="dmr-lan-port-help"/><small id="dmr-lan-port-help">默认 3082（主服务 3080、手机接力 3081 之外的独立端口）。</small></label>
        <div className="dmr-security"><strong>安全提示</strong><p>直连模式没有登录认证，等同以 0.0.0.0 启动 dsh web（官方 CLI 出于安全默认拒绝该旗标）。只应在可信局域网内开启，用完后请关闭；关闭即终止子实例进程。</p></div>
      </div>
      <button type="button" className="dmr-primary" onClick={startLan} disabled={lanBusy === 'start'}>{lanBusy === 'start' ? '正在启动子实例…' : <><RelayIcon name="link"/>开启局域网直连（0.0.0.0）</>}</button>
    </div>}
    {lanMessage && <div className="dmr-message error" role="alert">{lanMessage}</div>}
  </section>
}
