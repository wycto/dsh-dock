...
1. **设置账号密码并开启**：功能坞「远程访问」页填账号密码点开启，网关在 3081 端口（可改）监听 `0.0.0.0`——这是唯一的远程入口，主实例保持仅本机，结构上不存在免登录的直连路径。
2. **设备登录**：任何设备（电脑/手机/平板）访问 `http://<电脑IP>:3081`，自动跳转登录页，账号密码登录后使用与本机完全一致的 DSH——同一个实例，会话、正在进行的任务进度、设置实时一致。会话 Cookie 7 天滑动过期；「退出登录」按钮随时注销。
...
主 DSH 默认只监听 `127.0.0.1`（官方 CLI 拒绝 `--host 0.0.0.0`，因为浏览器表层没有登录认证）。手机接力提供两种局域网接入方式，二者都基于**服务器模式（单实例）**——一个 DSH 进程服务所有设备，会话、正在进行的任务进度、设置、功能坞完全一致：

1. **服务器模式（0.0.0.0 单实例）**：在功能坞「手机接力」页点「开启服务器模式」，插件把 0.0.0.0 绑定写进用户 profile 补丁层（`~/.dsh/profiles/web/cordis.patch.yml`），重启 `dsh web` 后主实例直接服务局域网：
   - 本机 `127.0.0.1` 与局域网设备访问**同一个进程**——正在进行的任务在所有设备上实时一致（不存在双实例方案的任务状态分叉）；
   - **工作区文件夹选择**自动切换为浏览器浏览模式（DSH 按 0.0.0.0 绑定自适应）：任何设备都能在浏览器里浏览/新建目录；顶部面包屑与铅笔图标支持直接输入路径（如 `F:\workspace\demo`）跨盘导航；
   - 面板展示局域网地址、二维码、复制链接；关闭即移除补丁（重启后恢复仅本机）；
   - ⚠️ **没有登录认证**，等同 `dsh web --host 0.0.0.0`——任何能访问该地址的人都能操作 DSH，只应在可信局域网/家用服务器开启。
2. **手机扫码接力**（受保护）：点「开启反向代理并生成二维码」，在独立端口（默认 3081）开启需要一次性链接认证的 HTTP/WebSocket 反向代理，上游即主实例。手机扫码进入的是同一个 DSH：历史会话、工作区（含浏览式选目录）、正在进行的任务、接力备注与电脑实时一致。令牌 10 分钟失效、只能用一次，关闭连接立即撤销手机端会话并停止端口监听。
