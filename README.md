...
| **任务动画** | 19 种任务运行动画（含星际远征、星云潮汐、曲速航道、量子雷达、星座网络、深海脉动、神经电路等）与完成/需确认通知（通知音、浏览器系统通知、钉钉/飞书群机器人推送），动画速度随任务活动联动，两组开关独立，配置全部持久化 |
...
1. 打开功能坞 → 任务动画，两组开关完全独立、按需组合：
   - **运行动画**：任务进行中才出现，8 种氛围模式任选（默认「流光细线」），**速度随任务吞吐自动加快**（Token 速率驱动）：
     ...
     - 星野：细碎星点飘移闪烁，带弯月与流星
     - 极光：屏幕顶部柔光带缓慢呼吸流动
   - **完成通知**：任务结束时右上角弹出通知卡片（标题 / 模型 / 耗时 / 回合步骤 / Token / 结果摘要 / 结束原因），可分别开关「完成 / 异常」通知、选择停留时长、开启浏览器系统通知（仅页面后台时推送）；**提示音** 6 种 WebAudio 合成音效可选（完成 / 异常 / 确认各配一音，可单独试听）
   - **需确认提醒**：工具等待用户批准（如沙箱越界执行命令）时弹常驻通知卡片 + 确认音（页面后台时同步系统通知），人不在屏幕前任务也不会干等；等待 / 卡住状态卡片只对当前正在看的会话出现，停靠屏幕右上不挡正文
